...
public abstract class AbstractTranscodingDeliveryEngine(e, MI : MediaItem)
  : AbstractDeliveryEngine!(RI, MI)
  , DeliveryListener
{
  ...
  private static final Logger log = LoggerFactory.getLogger!(AbstractTranscodingDeliveryEngine);
  ...
  protected DeliveryContainer retrieveTranscodedResource(MI mediaItem, MediaFormatProfile selectedVersion, DeliveryQuality.QualityType selectedQuality, Double timeOffsetInSeconds, Double durationInSeconds, Client client)
  {
    ...
    if (trDef !is null)
    {
      String timeOffset = timeOffsetInSeconds !is null ? String.format("-%s-%s", cast(Object[])[ timeOffsetInSeconds, Double.valueOf(durationInSeconds !is null ? durationInSeconds.doubleValue() : 0.0) ]) : "";
      String subtitle = mediaItem.getDeliveryContext().getHardsubsSubtitlesFile() !is null ? String.format("-%s", cast(Object[])[ mediaItem.getDeliveryContext().getHardsubsSubtitlesFile().getIdentifier() ]) : "";
      String transcodingIdentifier = String.format("transcoding-temp-%s-%s-%s%s%s.%s", cast(Object[])[ mediaItem.getId(), client.getRendererProfile().getId(), selectedQuality.toString(), timeOffset, subtitle, "stf" ]);
