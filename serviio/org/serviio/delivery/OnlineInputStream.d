...
public class OnlineInputStream : InputStream
{
    ...
    private byte[] readFileChunk(long startByte, long byteCount)
    {
        ...
        {
            log.debug_(String_format("Byte range not supported for %s, returning the whole stream", cast(Object[])[ contentURL.toString() ]));
            ...
            log.debug_(String_format("302 returned, redirecting to %s", cast(Object[])[ contentURL ]));
            return readFileChunk(startByte, byteCount);
        }if (response.getStatus().equals(Status.CLIENT_ERROR_REQUESTED_RANGE_NOT_SATISFIABLE)) {
            wholeStream = getResponseStream(response);
            log.debug_(String_format("Byte range not satisfiable for %s, returning the whole stream", cast(Object[])[ contentURL.toString() ]));
            throw new RangeNotSupportedException();
        }
        throw new IOException(String_format("Status '%s' received from '%s', cancelling transfer", cast(Object[])[ response.getStatus(), contentURL.toString() ]));
    }
    ...
    private InputStream getResponseStream(Response response)
    {
        ...
        throw new IOException(String_format("Cannot open stream from '%s', possibly incorrect URL or invalid HTTP response", cast(Object[])[ contentURL.toString() ]));
    }
}
