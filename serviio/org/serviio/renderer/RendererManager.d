...
public class RendererManager
{
    ...
    public void rendererAvailable(String uuid, String ipAddress, int timeToKeep, String descriptionURL, String server)
    {
        synchronized (activeRenderers) {
            ...
            if (activeRenderer is null)
            {
                ...
                if (renderer !is null)
                {
                    ...
                    try {
                        ...
                        log.warn(String_format("Error while retrieving renderer description: %s", cast(Object[])[ e.getMessage() ]));
                        return;
                    }
                }
                addRenderer = true;
            ...
            if (addRenderer) {
                log.debug_(String_format("Adding renderer %s to list of active renderers", cast(Object[])[ renderer ]));
            }
    ...
    public synchronized void rendererAvailable(Header[] httpHeaders, String ipAddress)
    {
        try
        {
            ...
            log.debug_(String_format("Looking for a renderer profile for Http headers: %s", cast(Object[])[ HttpUtils.headersToString(httpHeaders) ]));
            ...
            if (existingRenderer is null)
            {
                ...
                log.debug_(String_format("Updating renderer on IP %s (forced: %s, profile: %s) with profile %s", cast(Object[])[ ipAddress, Boolean.valueOf(existingRenderer.isForcedProfile()), existingRenderer.getProfileId(), renderer.getProfileId() ]));
    ...
    public void rendererUnavailable(String uuid) {
        synchronized (activeRenderers) {
            if (activeRenderers.containsKey(uuid)) {
                log.debug_(String_format("Removing renderer %s from list of active renderers", cast(Object[])[ uuid ]));
    ...
    public void searchForActiveRenderers() {
        try {
            ...
            log.warn(String_format("Exception during searching for active renderers: %s", cast(Object[])[ e.getMessage() ]));
        }
    }
    ...
    public void createRenderer(Renderer renderer)
    {
        ...
        log.info(String_format("Stored a new renderer: uuid='%s', name = '%s', ipAddress='%s', profile = '%s'", cast(Object[])[ renderer.getUuid(), renderer.getName(), renderer.getIpAddress(), renderer.getProfileId() ]));
    }
    ...
    {
        log.debug_(String_format("Retrieve device description from %s", cast(Object[])[ descriptionURL ]));
        try {
            ...
            throw new CannotResolveRendererProfileException(String_format("Cannot retrieve device description from %s", cast(Object[])[ descriptionURL ]), e);
        }
    }
    ...
    protected Renderer getRendererByRendererDescription(String uuid, String ipAddress, String serverName, String descriptionXML) {
        try {
            ...
            String modelName = XPathUtil.getNodeValue(deviceNode, String_format("%smodelName", cast(Object[])[ nameSpacePrefix ]), nsContext);
            String modelNumber = XPathUtil.getNodeValue(deviceNode, String_format("%smodelNumber", cast(Object[])[ nameSpacePrefix ]), nsContext);
            String productCode = XPathUtil.getNodeValue(deviceNode, String_format("%sUPC", cast(Object[])[ nameSpacePrefix ]), nsContext);
            String friendlyName = XPathUtil.getNodeValue(deviceNode, String_format("%sfriendlyName", cast(Object[])[ nameSpacePrefix ]), nsContext);
            String manufacturer = XPathUtil.getNodeValue(deviceNode, String_format("%smanufacturer", cast(Object[])[ nameSpacePrefix ]), nsContext);

            log.debug_(String_format("Looking for a renderer profile for: friendly name = '%s', model name= '%s', model number = '%s', manufacturer = '%s', product code = '%s', server name = '%s'", cast(Object[])[ friendlyName, modelName, modelNumber, manufacturer, productCode, serverName ]));
