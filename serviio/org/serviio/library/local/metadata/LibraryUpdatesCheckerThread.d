...
public class LibraryUpdatesCheckerThread : AbstractLibraryCheckerThread
{
    ...
    {
        log.debug_(String_format("Looking for removed/updated files in repository: %s", cast(Object[])[ repository.getFolder() ]));
        ...
        while ((workerRunning) && (itemsIt.hasNext()))
        {
            ...
            if ((mediaFile !is null) && (!mediaFile.exists())) {
                try {
                    log.info(String_format("Removing '%s' (%s) from Library", cast(Object[])[ item.getTitle(), mediaFile.getPath() ]));
                    ...
                    log.warn(String_format("Cannot remove item %s from Library. Message: %s", cast(Object[])[ item.getTitle(), e.getMessage() ]), e);
                }
                ...
                try {
                    ...
                    log.warn(String_format("Cannot search for updated metadata for title %s. Message: %s", cast(Object[])[ item.getTitle(), e.getMessage() ]), e);
                }
        }
    }
    ...
    {
        log.debug_(String_format("Looking for removed playlist files in repository: %s", cast(Object[])[ repository.getFolder() ]));
        ...
        while ((workerRunning) && (playlistIt.hasNext())) {
            ...
            if ((playlistFile !is null) && (!playlistFile.exists()))
                try {
                    log.info(String_format("Removing playlist '%s' (%s) from Library", cast(Object[])[ playlist.getTitle(), FileUtils.getProperFilePath(playlistFile) ]));

                    PlaylistService.detetePlaylistAndItems(playlist.getId());
                    notifyListenersRemove(playlistFile.getName());
                } catch (Exception e) {
                    log.warn(String_format("Cannot remove playlist %s from Library. Message: %s", cast(Object[])[ playlist.getTitle(), e.getMessage() ]), e);
                }
        }
    }
    ...
    protected void updateMetadata(Repository repository)
    {
        ...
        if (mediaItems.size() > 0) {
            log.debug_(String_format("Updating dirty metadata for repository: %s", cast(Object[])[ repository.getFolder() ]));
                    ...
                    if ((mediaFile !is null) && (mediaFile.exists())) {
                        ...
                        if ((mergedMetadata !is null) && (workerRunning))
                        {
                            ...
                            if ((fileType == MediaFileType.AUDIO) && (( cast(AudioMetadata)mergedMetadata !is null ))) {
                                ...
                                log.error(String_format("Error during update metadata extraction for file %s. Metadata mismatch.", new Object[0]), mediaFile.getName());
                            }
                            if (updated) {
                                log.info(String_format("Updated '%s' (%s) in Library", cast(Object[])[ mediaItem.getTitle(), mediaFile.getPath() ]));
                                notifyListenersUpdate(mediaFile.getName());
                            }
                        }
                    }
                } catch (InvalidMetadataException ime) {
                    log.warn(String_format("Cannot update file %s in library because of invalid metadata. Message: %s", cast(Object[])[ mediaItem.getTitle(), ime.getMessage() ]));
                } catch (Exception e) {
                    log.warn(String_format("Cannot update file %s in library because of an unexpected error. Message: %s", cast(Object[])[ mediaItem.getTitle(), e.getMessage() ]), e);
                }
            }
        }
    }
