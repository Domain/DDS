...
{
	private static immutable DateFormat DATE_FORMAT;
	private static immutable DateFormat DATE_FORMAT2;
		...
		if (fileType == MediaFileType.VIDEO)
		{
			...
			if ((folder !is null) && (folder.exists()) && (folder.isDirectory()))
			{
				...
				if (nfoFile !is null)
				{
					...
					if (validFile) {
						log.debug_(String_format("Found XBMC NFO file %s. Will try to extract metadata from it.", cast(Object[])[ nfoFile.getName() ]));
	...
	{
		log.debug_(String_format("Checking if file '%s' is a XBMC NFO file", cast(Object[])[ xmlFile.getName() ]));
		InputStream xmlStream = null;
		try {
			xmlStream = new FileInputStream(xmlFile);
			Node rootNode = XPathUtil.getRootNode(xmlStream);
			if (rootNode !is null) {
				...
				if ((movieNode !is null) || (tvNode !is null)) {
					log.debug_(String_format("File '%s' is a valid XBMC file", cast(Object[])[ xmlFile.getName() ]));
					return true;
				}
			}
			log.debug_(String_format("File '%s' is not a XBMC file", cast(Object[])[ xmlFile.getName() ]));
			return false;
		}
		catch (XPathExpressionException e)
		{
	...
	private void retrieveMovieMetadata(VideoMetadata metadata, Node movieNode) {
		...
		try {
			...
			throw new InvalidMediaFormatException(String_format("Error during parsing XBMC movie NFO file: %s", cast(Object[])[ e.getMessage() ]));
		}
	}
	...
	private void retrieveEpisodeMetadata(VideoMetadata metadata, Node tvNode, File mediaFile) {
		...
		try
		{
			...
			throw new InvalidMediaFormatException(String_format("Error during parsing XBMC TV NFO file: %s", cast(Object[])[ e.getMessage() ]));
		}
	}
	...
	private Node getSeriesFileRootNode(File mediaFile)
	{
		...
		try {
			...
			throw new IOException(String_format("File '%s' is not a XBMC show file", cast(Object[])[ seriesNfo.getPath() ]));
		} catch (XPathExpressionException e) {
			throw new IOException(String_format("File '%s' couldn't be parsed: %s", cast(Object[])[ seriesNfo.getPath(), e.getMessage() ]));
		} finally {
			FileUtils.closeQuietly(xmlStream);
		}
	}
	...
	private File findShowNfoFile(File mediaFile)
	{
		...
		if (seriesNfo is null) {
			throw new IOException(String_format("Cannot find tvshow.nfo file for %s, skipping XBMC metadata extraction", cast(Object[])[ mediaFile.getPath() ]));
		}
		return seriesNfo;
	}
