...
{
	private static /*Pattern[]*/enum EPISODIC_PATTERNS = [ Pattern.compile("[s]([\\d]+)[e]([\\d]+).*", 2), Pattern.compile("[s]([\\d]+)\\.[e]([\\d]+).*", 2), Pattern.compile("([\\d]+)[x]([\\d]+)", 2), Pattern.compile("[s]([\\d]+)_[e]([\\d]+).*", 2), Pattern.compile("\\b([0][0-9]|[1][0-8])([\\d]{2,2})\\b.*", 2), Pattern.compile("^([\\d])[\\s]([\\d]{2,2})", 2), Pattern.compile("^([\\d]{1,2})[\\s]([\\d]{2,2})", 2), Pattern.compile("([\\d]{1,2})[\\s]{0,1}-[\\s]{0,1}([\\d]{1,2})[\\s]*-", 2), Pattern.compile("season\\s([\\d]+)\\sepisode\\s([\\d]+).*", 2) ];

	private static /*Pattern[]*/enum SPECIAL_PATTERNS = [ Pattern.compile(".*trailer.*", 2), Pattern.compile(".*sample.*", 2) ];

	private static /*Pattern*/enum MOVIE_YEAR_PATTERN = Pattern.compile("(?<!^)(\\[|\\()?(\\b\\d{4}\\b)(\\]|\\)?)");

	private static /*Pattern*/enum SERIES_YEAR_PATTERN = Pattern.compile("(\\[|\\()(\\d{4})(\\]|\\))");
