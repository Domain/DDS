...
public class TheMovieDBSourceAdaptor : SearchSourceAdaptor
{
    ...
    private static immutable NamespaceContext namespaceContext;

    static this()
    {
        log = LoggerFactory.getLogger!(TheMovieDBSourceAdaptor);
        releaseDateFormat = new SimpleDateFormat("yyyy-MM-dd");
        namespaceContext = new OpenSearchNamespaceContext();
    }
    ...
    public void retrieveMetadata(String movieId, VideoMetadata videoMetadata)
    {
        ...
        if (ObjectValidator.isNotEmpty(movieXML))
        {
            try
            {
                ...
                Node movieNode = XPathUtil.getNode(rootMovieNode, "OpenSearchDescription/movies/movie");
                ...
                videoMetadata.setDescription(StringUtils.trim(XPathUtil.getNodeValue(movieNode, "overview")));
                String genre = XPathUtil.getNodeValue(movieNode, "categories/category[@type='genre'][1]/@name");
                if (ObjectValidator.isNotEmpty(genre))
                {
                    videoMetadata.setGenre(StringUtils.trim(genre));
                }
                videoMetadata.setDate(getReleaseDate(XPathUtil.getNodeValue(movieNode, "released")));
                videoMetadata.setActors(getCast(XPathUtil.getNodeSet(movieNode, "cast/person[@job='Actor']")));
                videoMetadata.setDirectors(getCast(XPathUtil.getNodeSet(movieNode, "cast/person[@job='Director']")));
                videoMetadata.setProducers(getCast(XPathUtil.getNodeSet(movieNode, "cast/person[@job='Producer']")));
                ...
                {
                    NodeList posters = XPathUtil.getNodeSet(movieNode, "images/image[@type='poster' and @size='cover']");
                    if (posters !is null)
                    {
                        for (int i = 0; i < posters.getLength(); i++)
                        {
                            Node posterNode = posters.item(i);
                            String posterURL = XPathUtil.getNodeValue(posterNode, "@url");
    ...
    private String searchForMovie(String[] movieNames, Integer releaseYear)
    {
        ...
        if (returnedMovieNodes.size() > 0)
        {
            try
            {
                List!(Node) moviesWithMatchingName = filterMovieNodesForNameMatch(returnedMovieNodes, movieNames);
    ...
    private List!(Node) findAllSearchMatches(String[] movieNames, Integer year)
    {
        ...
        foreach (String movieName ; movieNames)
        {
            if (ObjectValidator.isNotEmpty(movieName))
            {
                ...
                try
                {
                        ...
                        && (searchResultXML.startsWith("<?xml")))
                    {
                        ...
                        if (numberReturned.intValue() > 0)
                        {
                            ...
                            allReturnedNodes.addAll(XPathUtil.getListOfNodes(movieNodes));
                        }
                    }
                    else
                    {
                ...
                {
                    throw new IOException("Cannot retrieve movie search results, file not found");
                }
                catch (Exception e)
                {
                    throw new IOException(String_format("Cannot retrieve movie search results: %s", cast(Object[])[ e.getMessage() ]));
    ...
    private List!(Node) filterMovieNodesForNameMatch(List!(Node) movieNodes, String[] movieNames)
    {
        ...
        if ((movieNodes !is null) && (movieNodes.size() > 0))
        {
            for (int i = 0; i < movieNodes.size(); i++)
            {
                ...
                String originalName = XPathUtil.getNodeValue(movieNode, "original_name");
                NodeList altNamesNodes = XPathUtil.getNodeSet(movieNode, "alternative_name");
                ...
                for (int j = 0; j < altNamesNodes.getLength(); j++)
                {
                    ...
                    {
                        altNames.add(StringUtils.localeSafeToLowercase(altName).trim());
                    }
                }
                foreach (String movieName ; movieNames)
                {
                    if (movieName !is null)
                    {
                        ...
                        if (((name !is null) && (trimmedMovieName.equalsIgnoreCase(name.trim()))) || (altNames.contains(trimmedMovieName))
                            || ((originalName !is null) && (trimmedMovieName.equalsIgnoreCase(originalName.trim()))))
                        {
    ...
    {
        String title = StringUtils.trim(XPathUtil.getNodeValue(movieNode, "name"));
        if (Configuration.isMetadataUseOriginalTitle())
        {
            String originalTitle = StringUtils.trim(XPathUtil.getNodeValue(movieNode, "original_name"));
