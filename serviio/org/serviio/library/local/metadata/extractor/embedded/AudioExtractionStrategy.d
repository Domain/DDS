...
public abstract class AudioExtractionStrategy
{
	...
	{
		log.debug_(String_format("Extracting metadata of audio file: %s", cast(Object[])[ f.getFile().getAbsolutePath() ]));
		...
		if (header !is null) {
			...
			try {
				...
				log.debug_(String_format("Invalid bitrate: %s", cast(Object[])[ Long.valueOf(bitrate) ]));
			}
