...
public abstract class ImageExtractionStrategy
{
	...
	{
		log.debug_(String_format("Extracting metadata of image file: %s", cast(Object[])[ f.getFilename() ]));
		try
		{
			...
			throw new IOException(String_format("Cannot read file %s for metadata extraction: %s", cast(Object[])[ f.getFilename(), e.getMessage() ]), e);
		}
		catch (OutOfMemoryError e)
		{
			throw new IOException(String_format("Cannot read file %s for metadata extraction because of out-of-memory error", cast(Object[])[ f.getFilename() ]), e);
		}
	}
