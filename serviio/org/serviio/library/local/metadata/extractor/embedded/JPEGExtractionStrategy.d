...
public class JPEGExtractionStrategy : ImageExtractionStrategy
{
	...
	override public void extractMetadata(ImageMetadata metadata, ByteSource f)
	{
		...
		try
		{
			...
			log.debug_(String_format("Cannot read file %s for metadata extraction. Message: %s", cast(Object[])[ f.getFilename(), e.getMessage() ]));
		} catch (ParseException e) {
		}
		catch (OutOfMemoryError e) {
			log.debug_(String_format("Cannot get metadata of file %s because of OutOfMemory error. The file is dodgy, but will still be added to the library.", cast(Object[])[ f.getFilename() ]));
		} catch (IOException e) {
			log.debug_(String_format("Cannot read EXIF metadata for file %s. Message: %s", cast(Object[])[ f.getFilename(), e.getMessage() ]));
		}
	}
