...
public class CoverImageInFolderExtractor : AbstractLocalFileExtractor
{
	...
	private static const String[] VIDEO_FILES = [ "dvdcover.jpg", "folder.jpg", "movie.jpg", ".*poster.*.jpg" ];
		...
		if ((fileType == MediaFileType.AUDIO) || (fileType == MediaFileType.VIDEO)) {
			...
			if ((folder !is null) && (folder.exists()) && (folder.isDirectory()))
			{
				...
				if (foundFiles.length > 0)
				{
					...
					log.debug_(String_format("Found cover image %s", cast(Object[])[ imageFile.getName() ]));
	...
	override protected void retrieveMetadata(LocalItemMetadata metadata, File imageFile, File mediaFile)
	{
		try
		{
			...
			throw new InvalidMediaFormatException(String_format("Cannot read cover image %s: %s", cast(Object[])[ imageFile.getName(), e.getMessage() ]));
		}
	}
