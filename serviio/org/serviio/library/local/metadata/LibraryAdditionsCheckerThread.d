    ...
    protected void searchForNewFiles(File folder, Repository repository)
    {
        ...
        if (workerRunning)
            if ((folder.isDirectory()) && (folder.canRead())) {
                log.debug_(String_format("Looking for files to share in folder: %s", cast(Object[])[ folder.getAbsolutePath() ]));
                ...
                while ((workerRunning) && (files !is null) && (i < files.length)) {
                    ...
                    if (file.isDirectory()) {
                        ...
                        log.warn(String_format("Cannot add file %s because of invalid metadata. Message: %s", cast(Object[])[ file.getName(), ime.getMessage() ]));
                    } catch (CannotParsePlaylistException cppe) {
                        log.warn(String_format("Cannot add playlist file %s. Message: %s", cast(Object[])[ file.getName(), cppe.getMessage() ]), cppe);
                    } catch (Exception e) {
                        log.warn(String_format("Cannot add file %s because of an unexpected error. Message: %s", cast(Object[])[ file.getName(), e.getMessage() ]), e);
                    }
                }
            }
            else {
                log.warn(String_format("Folder '%s' is either not an existing directory or cannot be read due to access rights", cast(Object[])[ folder.getAbsolutePath() ]));
            }
    }

    private void addNewMediaFile(Repository repository, File file, MediaFileType fileType)
    {
        log.debug_(String_format("Found file '%s', checking if it's already in the Library", cast(Object[])[ file.getName() ]));
            ...
            if (file.canRead()) {
                ...
                if ((mergedMetadata !is null) && (workerRunning))
                {
                    ...
                    if ((fileType == MediaFileType.AUDIO) && (( cast(AudioMetadata)mergedMetadata !is null ))) {
                        ...
                        log.error(String_format("Error during metadata extraction for file %s. Metadata mismatch.", new Object[0]), file.getName());
                    }
                    if (newAdded) {
                        log.info(String_format("Added file '%s' (title: %s) to Library", cast(Object[])[ file.getName(), mergedMetadata.getTitle() ]));
                        notifyListenersAdd(file.getName());
                    }
                }
            } else {
                log.warn(String_format("File '%s' cannot be read, probably due to access rights", cast(Object[])[ file.getAbsolutePath() ]));
            }
    }

    private void addNewPlaylistFile(Repository repository, File file)
    {
        log.debug_(String_format("Found playlist file '%s', checking if it's already in the Library", cast(Object[])[ file.getName() ]));
        ...
        if ((!playlistPresent) && (file.exists()) && (workerRunning))
            if (file.canRead()) {
                ...
                if (playlist !is null) {
                    ...
                    log.info(String_format("Added playlist '%s' (title: %s) to Library", cast(Object[])[ file.getName(), playlist.getTitle() ]));
                    notifyListenersAdd(file.getName());
                }
            } else {
                log.warn(String_format("Playlist file '%s' cannot be read, probably due to access rights", cast(Object[])[ file.getAbsolutePath() ]));
            }
    }
