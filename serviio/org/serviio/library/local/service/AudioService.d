...
public class AudioService : Service
{
    ...
    static this()
    {
        log = LoggerFactory.getLogger!(AudioService)();
    }
    ...
    public static void addMusicTrackToLibrary(AudioMetadata metadata, Repository repository)
    {
        Long mediaItemId;
        if (metadata !is null) {
            log.debug_(String_format("Adding music track into database: %s", cast(Object[])[ metadata.getTitle() ]));
    ...
    public static void removeMusicTrackFromLibrary(Long mediaItemId)
    {
        ...
        if (track !is null) {
            log.debug_(String_format("Removing music track from database: %s", cast(Object[])[ track.getTitle() ]));
    ...
    {
        if (metadata !is null) {
            log.debug_(String_format("Updating music track in database: %s", cast(Object[])[ metadata.getTitle() ]));
    ...
    private static Long findOrCreateAlbum(AudioMetadata metadata)
    {
        if (ObjectValidator.isNotEmpty(metadata.getAlbum()))
        {
            ...
            if (album is null) {
                log.debug_(String_format("Album %s (%s) not found, creating a new one", cast(Object[])[ metadata.getAlbum(), albumArtist ]));
            ...
            log.debug_(String_format("Album %s found, attaching the music track to it", cast(Object[])[ album.getTitle() ]));
            return album.getId();
        }
