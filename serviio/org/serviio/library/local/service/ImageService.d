...
public class ImageService : Service
{
    ...
    public static void addImageToLibrary(ImageMetadata metadata, Repository repository)
    {
        Long mediaItemId;
        if (metadata !is null) {
            log.debug_(String_format("Adding image into database: %s", cast(Object[])[ metadata.getTitle() ]));
    ...
    public static void removeImageFromLibrary(Long mediaItemId)
    {
        ...
        if (image !is null) {
            log.debug_(String_format("Removing image from database: %s", cast(Object[])[ image.getTitle() ]));
    ...
    {
        if (metadata !is null) {
            log.debug_(String_format("Updating image in database: %s", cast(Object[])[ metadata.getTitle() ]));
    ...
    private static Long createThumbnail(ImageMetadata metadata)
    {
        if (Configuration.isGenerateLocalThumbnailForImages()) {
            try {
                ...
                log.warn(String_format("Cannot convert image to JPG. Message: %s", cast(Object[])[ e.getMessage() ]));
                return null;
            }
        }
        return null;
