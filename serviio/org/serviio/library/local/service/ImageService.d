...
public class ImageService : Service
{
    ...
    static this()
    {
        log = LoggerFactory.getLogger!(ImageService);
    }
    ...
    public static void addImageToLibrary(ImageMetadata metadata, Repository repository)
    {
        if (metadata !is null)
        {
            log.debug_(java.lang.String.format("Adding image into database: %s", cast(Object[])[ metadata.getTitle() ]));
            ...
            Image image = new Image(metadata.getTitle(), metadata.getContainer(), new File(metadata.getFilePath()).getName(), metadata.getFilePath(), Long.valueOf(metadata.getFileSize()), cast(Long)folderHierarchy.getValueA(), repository.getId(), metadata.getDate());
            ...
            image.setDirty(metadata.isDirty());


            Long mediaItemId = Long.valueOf(DAOFactory.getImageDAO().create(image));
            ...
            SearchService.makeImageSearchable(mediaItemId, image, repository, folderHierarchy);
        }
        else
        {
    ...
    public static void removeImageFromLibrary(Long mediaItemId)
    {
        ...
        if (image !is null)
        {
            log.debug_(java.lang.String.format("Removing image from database: %s", cast(Object[])[ image.getTitle() ]));
            ...
            SearchService.makeImageUnsearchable(mediaItemId, image);
        }
        else
        {
    ...
    public static void updateImageInLibrary(ImageMetadata metadata, Long mediaItemId)
    {
        if (metadata !is null)
        {
            log.debug_(java.lang.String.format("Updating image in database: %s", cast(Object[])[ metadata.getTitle() ]));
            ...
            updatedImage.setDirty(metadata.isDirty());
            ...
            SearchService.makeImageSearchable(mediaItemId, updatedImage, RepositoryService.getRepository(updatedImage.getRepositoryId()), FolderService.getFolderHierarchy(updatedImage.getFolderId()));
        }
        else
        {
        ...
        if (Configuration.isGenerateLocalThumbnailForImages()) {
            try
            {
                ...
                if (coverImage is null) {
                    coverImage = ImageUtils.loadImage(new File(metadata.getFilePath()));
                }
                return CoverImageService.createCoverImage(coverImage, metadata.getExifRotation());
            }
            catch (Throwable e)
            {
                log.warn(java.lang.String.format("Cannot convert image to JPG. Message: %s", cast(Object[])[ e.getMessage() ]));
                return null;
            }
        }
        return null;
