...
public class TechnicalMetadataCacheDecorator
  ...
  , OnlineCacheDecorator!(TechnicalMetadata)
{
  ...
  public void store(String url, TechnicalMetadata cachedValue)
  {
    try
    {
      ...
      this.log.debug_(String.format("Stored technical metadata for online item '%s' in the cache (%s), returning it", cast(Object[])[ url, this.regionName ]));
    }
  ...
  public void evict(String url)
  {
    try
    {
      ...
      this.log.debug_(String.format("Removed technical metadata fro online item '%s' from cache (%s)", cast(Object[])[ url, this.regionName ]));
    }
    catch (CacheException e)
    {
      this.log.warn(String.format("Could not remove feed %s from cache (%s): %s", cast(Object[])[ url, this.regionName, e.getMessage() ]));
    }
  }
}
