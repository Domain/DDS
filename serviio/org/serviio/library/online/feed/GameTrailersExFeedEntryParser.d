...
public class GameTrailersExFeedEntryParser : FeedEntryParser
{
	private static Logger log;

	static this()
	{
		log = LoggerFactory.getLogger!(GameTrailersExFeedEntryParser)();
	}
		...
		if (mod !is null)
		{
			if (mod.getThumbnailUrl() !is null) {
				try {
					...
					log.debug_(String_format("Invalid thumbnail URL: %s. Message: %s", cast(Object[])[ mod.getThumbnailUrl(), e.getMessage() ]));
				}
			}
