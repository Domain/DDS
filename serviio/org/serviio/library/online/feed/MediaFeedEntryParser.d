  ...
  : FeedEntryParser
{
  ...
  {
    MediaEntryModule module = cast(MediaEntryModule)entry.getModule("http://search.yahoo.com/mrss/");
    if (module !is null)
    {
      Metadata md = findMetadata(module);
      MediaContent con = findContents(module);
  ...
  {
    if ((module.getMediaGroups() !is null) && (module.getMediaGroups().length > 0)) {
  ...
  private MediaContent findContents(MediaEntryModule module)
  {
    ...
    else if ((module.getMediaContents() !is null) && (module.getMediaContents().length > 0))
    {
    ...
    if (availableContents !is null)
    {
      ...
      if (content is null)
      {
        ...
        Arrays.sort(sortedContent, new MediaContentComparator(null));
