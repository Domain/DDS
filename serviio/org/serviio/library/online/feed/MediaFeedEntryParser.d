...
public class MediaFeedEntryParser : FeedEntryParser
{
	...
	private void setThumbnail(MediaContent con, Metadata md, FeedItem item) {
		...
		if ((availableThumbnails !is null) && (availableThumbnails.length > 0)) {
			...
			try
			{
				...
				log.warn(String_format("Malformed url of a media feed thumbnail (%s), skipping this thumbnail", cast(Object[])[ selectedThumbnail.getUrl().toString() ]));
			}
		}
	}
	...
	private class MediaContentComparator : Comparator!(MediaContent)
	{
		private this()
		{
		}
