...
public abstract class AbstractUrlExtractor
{
  ...
  public static ContentURLContainer extractItemUrl(AbstractUrlExtractor plugin, OnlineItem item)
  {
    if (plugin !is null)
    {
      ...
      if (( cast(WebResourceUrlExtractor)plugin !is null )) {
        return (cast(WebResourceUrlExtractor)plugin).extractItemUrl((cast(WebResourceFeedItem)item).toContainerItem());
      }
    }
    return null;
  }
  ...
  {
    Encoding responseEncoding = response.getEntity().getEncodings().size() > 0 ? (Encoding)response.getEntity().getEncodings().get(0) : null;
  ...
  public override hash_t toHash()
  {
    int prime = 31;
    int result = 1;
    result = 31 * result + (getExtractorName() is null ? 0 : getExtractorName().hashCode());
    return result;
  }
