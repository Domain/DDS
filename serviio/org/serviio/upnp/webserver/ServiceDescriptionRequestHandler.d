...
public class ServiceDescriptionRequestHandler : AbstractDescriptionRequestHandler
{
    override protected void handleRequest(HttpRequest request, HttpResponse response, HttpContext context)
    {
        ...
        if (serviceShortName !is null) {
            log.debug_(String_format("ServiceDescription request received for service %s", cast(Object[])[ serviceShortName ]));
            ...
            if (service !is null)
            {
                ...
                if (message !is null)
                {
                    ...
                    log.debug_(String_format("Service with name %s is not supported, sending back 404 error", cast(Object[])[ serviceShortName ]));
                }
            }
            else {
                response.setStatusCode(404);
                log.debug_(String_format("Service with name %s doesn't exist in the root device, sending back 404 error", cast(Object[])[ serviceShortName ]));
            }
        }
    }
}
