...
public abstract class AbstractImagesRetrievalCommand : AbstractEntityItemCommand!(Image)
{
    ...
    {
        return new HashSet(Arrays.asList(cast(ObjectClassType[])[ ObjectClassType.IMAGE_ITEM, ObjectClassType.PHOTO ]));
    }

    override protected Image retrieveSingleEntity(Long entityId)
    {
        Image image = ImageService.getImage(entityId);
        return image;
    }

    override protected String getItemTitle(Image image, bool markItem)
    {
