...
public class ListSongsForAlbumCommand : AbstractSongsRetrievalCommand
{
	...
	{
		List!(MusicTrack) songs = AudioService.getListOfSongsForAlbum(new Long(getInternalObjectId()), accessGroup, startIndex, count);
		return songs;
	}

	public int retrieveItemCount()
	{
		return AudioService.getNumberOfSongsForAlbum(new Long(getInternalObjectId()), accessGroup);
	}
}
