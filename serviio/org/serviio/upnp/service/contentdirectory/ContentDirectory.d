  ...
  : Service
{
    ...
    else
    {
      ...
      else
      {
        ...
        if ((object !is null) && (object.getEntityId() !is null))
        {
          ...
          log.warn(String.format("ObjectId %s is not recognized, will not bookmark the last viewed position", cast(Object[])[ objectID ]));
        }
      }
    }
    return result;
  ...
  private !(T : ContentDirectoryMessageBuilder) ContentDirectoryMessageBuilder instantiateMessageBuilder(String filter, Profile rendererProfile)
  {
