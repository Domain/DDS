...
public class ContentDirectoryDefinitionParser
{
        ...
        for (int i = 0; i < childNodes.getLength(); i++)
        {
            ...
            else
            {
                if (childNode.getLocalName().equals(TAG_ACTION))
                    ...
                    throw new ContentDirectoryDefinitionException(String_format("Unsupported tag encountered: %s", cast(Object[])[ childNode.getLocalName() ]));
            }
    ...
    private static ContainerNode createContainerNode(Node node, DefinitionNode parent)
    {
        ...
        log.debug_(String_format("Found Container node with attributes: id=%s, titleKey=%s, class=%s, cacheRegion=%s, browsable = %s, editable = %s", cast(Object[])[ nodeId, nodeTitleKey, nodeContainerClass, nodeCacheRegion, nodeBrowsable, nodeEditable ]));
        try
        {
            ...
            throw new ContentDirectoryDefinitionException(String_format("Object class %s doesn't exist", cast(Object[])[ nodeContainerClass ]), e);
        }
    }
    ...
    private static ActionNode createActionNode(Node node, DefinitionNode parent)
    {
        ...
        log.debug_(String_format("Found Action node with attributes: command=%s, containerClass=%s, itemClass=%s, idPrefix=%s, recursive=%s, cacheRegion=%s", cast(Object[])[ nodeCommandClass, nodeContainerClass, nodeItemClass, nodeIdPrefix, Boolean.valueOf(recursive), nodeCacheRegion ]));
        try
        {
            ...
            throw new ContentDirectoryDefinitionException(String_format("Object class doesn't exist: %s", cast(Object[])[ e.getMessage() ]), e);
        }
    }
}
