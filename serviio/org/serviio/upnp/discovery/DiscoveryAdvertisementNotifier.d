...
public class DiscoveryAdvertisementNotifier : Multicaster , Runnable
{
    ...
    static this()
    {
        log = LoggerFactory.getLogger!(DiscoveryAdvertisementNotifier)();
    }
        ...
        while (workerRunning) {
            try {
                ...
                if (sent) {
                    ...
                    delay -= sendDuration;
                    log.debug_(String_format("Will advertise again in %s (advertisement duration is %s sec.)", cast(Object[])[ DateUtils.timeToHHMMSS(delay), Integer.valueOf(advertisementDuration) ]));
    ...
    public bool sendAlive()
    {
        ...
        bool sent = false;
        log.debug_(String_format("Found %s network interfaces to advertise on", cast(Object[])[ Integer.valueOf(ifaces.size()) ]));
        ...
        if (address !is null)
            try
            {
                ...
                if ((socket !is null) && (socket.isBound())) {
                    log.debug_(String_format("Multicasting SSDP alive using interface %s and address %s, timeout = %s", cast(Object[])[ MultiCastUtils.getInterfaceName(multicastInterface), address.getHostAddress(), Integer.valueOf(socket.getSoTimeout()) ]));
                    ...
                    log.debug_(String_format("Sending %s 'alive' messages describing device %s", cast(Object[])[ Integer.valueOf(messages.size()), device.getUuid() ]));
    ...
    private void sendByeByeToSingleInterface(NetworkInterface multicastInterface)
    {
        ...
        if (address !is null)
            try
            {
                socket = MultiCastUtils.startMultiCastSocketForSending(multicastInterface, address, 32);
                if ((socket !is null) && (socket.isBound())) {
                    log.debug_(String_format("Multicasting SSDP byebye using interface %s and address %s, timeout = %s", cast(Object[])[ MultiCastUtils.getInterfaceName(multicastInterface), address.getHostName(), Integer.valueOf(socket.getSoTimeout()) ]));
