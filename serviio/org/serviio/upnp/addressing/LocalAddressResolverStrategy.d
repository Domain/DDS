...
{
    private static immutable String BOUND_ADDRESS;
    ...
    static this()
    {
        BOUND_ADDRESS = System.getProperty("serviio.boundAddr");
        INVALID_NIC_NAMES = Arrays.asList(cast(String[])[ "vnic", "wmnet", "vmware", "bluetooth", "virtual" ]);
        log = LoggerFactory.getLogger!(LocalAddressResolverStrategy)();
    }
    ...
    private InetAddress convertStringToIPAddress(String address)
    {
        try
        {
            ...
            log.warn(String_format("Cannot resolve IP address %s, will try other means", cast(Object[])[ address ]));
        }
        return null;
    }
