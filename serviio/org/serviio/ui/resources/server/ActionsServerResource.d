...
public class ActionsServerResource : AbstractServerResource , ActionsResource
{
    private static immutable Logger log;
    
    static this()
    {
        log = LoggerFactory.getLogger!(ActionsServerResource)();
    }
    ...
    public ResultRepresentation execute(ActionRepresentation representation)
    {
        if (representation is null) {
            ...
            log.debug_(String_format("Action with name '%s' was requested", cast(Object[])[ representation.getName() ]));
            ...
            log.error(String_format("Action with name '%s' is not implemented, returning 400", cast(Object[])[ representation.getName() ]));
            setStatus(Status.CLIENT_ERROR_BAD_REQUEST);
        }
