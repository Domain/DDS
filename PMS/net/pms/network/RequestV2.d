...
public class RequestV2 : HTTPResource {
	private static Logger LOGGER = LoggerFactory.getLogger!RequestV2();
		...
		if ((method.opEquals("GET") || method.opEquals("HEAD")) && argument.startsWith("console/")) {
			...
			if (files.size() == 1) {
				...
				if (fileName.startsWith("thumbnail0000")) {
					...
					if (range.getStart() is null && splitRange.getStart() !is null) {
						range.setStart(splitRange.getStart());
					}

					if (range.getEnd() is null && splitRange.getEnd() !is null) {
			...
			if (argument.opEquals("description/fetch")) {
				...
				if (PMS.get().getServer().getHost() !is null) {
